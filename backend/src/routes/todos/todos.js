...
// View the todo (protected)
router.get('/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;
  try {
    const [todos] = await db.promise().query('SELECT * FROM todo WHERE id = ? AND user_id = ?', [id, req.user.id]);
    if (todos.length === 0) {
...
router.put('/:id', authenticateToken, async (req, res) => {
  ...
  try {
    const [existingTodo] = await db.promise().query('SELECT user_id FROM todo WHERE id = ?', [id]);
