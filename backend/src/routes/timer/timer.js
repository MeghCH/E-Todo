...
router.post('/start', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const [session] = await db.promise().query('SELECT * FROM timer_sessions WHERE id = ?', [result.insertId]);
...
router.post('/stop', authenticateToken, async (req, res) => {
  try {
    ...
    const [updatedSession] = await db.promise().query('SELECT * FROM timer_sessions WHERE id = ?', [session.id]);
...
router.delete('/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;
  try {
    const [existing] = await db.promise().query('SELECT user_id FROM timer_sessions WHERE id = ?', [id]);
...
router.delete('/', authenticateToken, async (req, res) => {
  try {
    await db.promise().query('DELETE FROM timer_sessions WHERE user_id = ?', [req.user.id]);
...
// Get all users' timer histories (manager only)
