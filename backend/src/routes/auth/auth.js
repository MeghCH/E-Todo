...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const [existingUser] = await db.promise().query('SELECT id FROM user WHERE email = ?', [email]);
    ...
    // Generate JWT
    const token = jwt.sign({ id: result.insertId, email }, process.env.SECRET, { expiresIn: '1h' });

    // Get user info including role
    const [userInfo] = await db.promise().query('SELECT id, email, name, firstname, role FROM user WHERE id = ?', [result.insertId]);
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Get user info including role
    const [userInfo] = await db.promise().query('SELECT id, email, name, firstname, role FROM user WHERE id = ?', [user.id]);
