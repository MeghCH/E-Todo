        ...
        {sessions.map((s) => (
          ...
          >
            <p className="text-sm text-neutral-500 mb-1">
              {s.userName ? s.userName : `Employé ${s.userId || "inconnu"}`}
            </p>
            <p className="font-mono">
              Début : {format(new Date(s.start), "Pp", { locale: fr })}
            </p>
            <p className="font-mono">
              Fin : {format(new Date(s.end), "Pp", { locale: fr })}
            </p>
            <p className="font-semibold">
              Durée : {(s.durationMs / 60000).toFixed(1)} min
            </p>
            {s.note && (
